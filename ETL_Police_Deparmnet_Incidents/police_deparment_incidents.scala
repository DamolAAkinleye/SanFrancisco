...
def dimension_Categorias(data: DataFrame)//:DataFrame = df
{
    ...
    //dfView.show(2000)
    create_hive_table(dim,"POLICE_DIM_CATEGORIA")
}
...
def dimension_Barrio(data:DataFrame)
{
    ...
    dfView = dfView.drop("direccion")
                    ...
                    .drop("PdDistrict")
    //dfView.show()
    create_hive_table(dim,"POLICE_DIM_BARRIO")
}
...
def dimension_Resoluciones(data:DataFrame)
{
    ...
    //dfView.show()
    create_hive_table(dim,"POLICE_DIM_RESOLUCIONES")
}
...
def police_factTable(data:DataFrame)
{
    ...
    //Dimension Categoria
    var dimension_categoria = hc.sql("select id, Category from police_dim_categoria")
    ...
    var dimension_resolution = hc.sql("select id, Resolution as resolucion from police_dim_resoluciones")
    ...
    factTable = factTable.withColumnRenamed("id", "idResolution")
                        ...
                        .drop("resolucion")
    //Dimension Barrio
    var dimension_barrio = hc.sql("select direccion, id from police_dim_barrio")
    ...
    factTable = factTable.withColumnRenamed("id","idDireccion")
                        ...
                        .drop("Address")
    //Dimension hora
    var dimension_hora = hc.sql("select id,horas from dim_horas")
    ...
    create_hive_table(factTable,"POLICE_FACTTABLE")
}
